 ...
 * 系统用户相关的controller
 */
infopowerWebApp.controller('sysUserCtrl', ['$scope','$http','$controller','$routeParams','$stateParams',
    function($scope,$http,$controller,$routeParams,$stateParams) {
        ...
        function updateSysUser(userid){
            ...
            $("#userDepartment").val($scope.user.userDepartment);
            var userflag=$scope.user.userFlag;
            if("1"==userflag){
            	//ldap用户
            	$(".ldapinfo").attr("disabled",true);
            }else{
            	$(".ldapinfo").attr("disabled",false);
            }
        }
        ...
        //普通用户与ldap用户切换操作
        $scope.dapUser = function(){
            if($scope.user.userFlag=='0'){
            	$(".ldapinfo").attr("disabled",false);
            	$(".ldapinfo").val("");
                $("#passwordNone1").css("display","");
                $("#passwordNone2").css("display","");
            }else{
                $("#passwordNone1").css("display","none");
                $("#passwordNone2").css("display","none");
                $scope.getApi('sysUsers/getlDapUser.do',{uid:$("#useraccount").val()},function(data){
                	if(data==""){
                		//查询不到LDAP帐户信息
                		$(".ldapinfo").val("");
                		$("#pid").val("0");
                		$('#hintInfo').html($.i18n.prop('page_ldap_error_search'));
						$('#successModal').modal('show');
                	}else{
                		//将姓名处理一下
                		var firstname=data.cn;
                		var lastname=data.sn;
                		if(firstname.startsWith(lastname)){
                			firstname=firstname.substring(lastname.length,firstname.length);
                		}
                		$("#firstName").val(firstname);
		                $("#userEmail").val(data.mail);
		                $("#lastName").val(lastname);
		                $("#userMobile").val(data.mobile);
		                $("#userDepartment").val(data.o);
		                $scope.user.firstName=firstname;
		                $scope.user.userEmail=data.mail;
		                $scope.user.lastName=lastname;
		                $scope.user.userMobile=data.mobile;
		                $scope.user.userDepartment=data.o;
		                $(".ldapinfo").attr("disabled",true);
                	}
                });
            }
