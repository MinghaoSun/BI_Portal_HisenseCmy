 ...
 * 系统菜单相关的controller
 */
infopowerWebApp.controller('sysReportCtrl', ['$scope','$http','$controller','$routeParams','$stateParams',
    function($scope,$http,$controller,$routeParams,$stateParams) {
      ...
      //获取报表状态
    	...
    	$scope.designType =[];
    	//获取报表类型
    	$scope.getApi('sysDic/getSysDic.do',{dicCode:"report_design_type"},function(data){
	        for (var i in data.list) {
	        	var x_id=data.list[i].dicValue;
	        	var obj;
	        	if (x_id=='0'){
	        		obj={
			                name: $scope.i18n('page_report_designType_jicheng'),
			                id:x_id		        				
	        		};
		        	$scope.designType.push(obj);
	        	}
	        	if (x_id=='1'){
	        		obj={
			                name: $scope.i18n('page_report_designType_diy'),
			                id:x_id		        				
	        		};
		        	$scope.designType.push(obj);
	        	}
      ...
      //获取报表工具
        ...
        $scope.addMenu = function(){
        	...
        	$("#selInput").val("");
        	$scope.sysMenu= {menuAttribute:$scope.menuAttributes[0].id,menuStatus:$scope.menuStatus[0].id,designType:$scope.designType[1].id};
        	getSysMenuParent();
