 ...
 * 系统菜单相关的controller
 */
infopowerWebApp.controller('sysDefaultReportSettingCtrl', ['$scope','$http','$controller','$routeParams','$stateParams',
    function($scope,$http,$controller,$routeParams,$stateParams) {
      ...
      //获取报表状态
        ...
        $scope.getMenuList = function(){
        	...
        	$scope.queryData('sysUserPersonalizedSetting/getReportSettingListByUserId.do?settingType=02',p,$scope.search);
        };
        ...
        function getMenuListBtn(isLoged){
			...
			*/
			...
			//$scope.queryData('sysMenu/getSysMenuListByPage.do',p,$scope.search);
			$scope.queryData('sysUserPersonalizedSetting/getReportSettingListByUserId.do?settingType=02&menuId='+$scope.menuid,p,$scope.search);

		}

        //确认默认报表设置
        $scope.confirmDefaultReportSetting = function(menuId){
			$('#btnConfirmDefaultSettingReport').one('click',function(){
				$scope.postApi('sysUserPersonalizedSetting/confirmSettingReport.do',{menuId:$scope.menuid,confirmmenuId:menuId,settingType:'02'},function(result){
		...
		//取消默认报表设置
		$scope.cancelDefaultReportSetting = function(menuId){
			$('#btnCancelDefaultConfirm').one('click',function(){
				$scope.postApi('sysUserPersonalizedSetting/cancelSettingReport.do',{menuId:$scope.menuid,cancelmenuId:menuId,settingType:'02'},function(result){
